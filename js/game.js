...
function renderUsers() {
    ...
    ownArray.forEach(name=> {
        ...
        if (name != currPlayer) {
            output+=`<div class="${extraClass}" onclick="whichPlayersCardsAreShown='${name}';renderAll()">${name}</div>`
        }
        else {
            output+=`<div class="current  ${extraClass}"  onclick="whichPlayersCardsAreShown='${name}';renderAll()">${name}</div>`
        }
    })
    //console.log(output);
